...
import { ImageContent, Message, TextContent } from "@/lib";

const MAX_RETRIES = 3;
const RETRY_DELAY = 5000;

export async function POST(req: NextRequest) {
  const { url, audience, market, insights, imageUrls } = await req.json();
  console.log("called");
  ...
  const imageContents: ImageContent[] = imageUrls.map((url: string) => ({
    type: "image_url",
    image_url: { url },
  }));
...
async function createMessage(
  ...
  message: { role: "user" | "assistant"; content: any[] },
) {
